...
Senior engineers don't religiously pick "OOP vs FP" — they reach for the right tool. This note covers the core OOP pillars and their failure modes, the FP toolkit (purity, immutability, higher-order functions), generics/parametric polymorphism, and how the two paradigms blend in modern code. The interview signal is *judgment*: when a pattern adds value and when it's ceremony.
...
### Key principles
- **SOLID** (covered in depth in Tier 6): SRP, OCP, LSP, ISP, DIP. For paradigms, **LSP** matters most — a subtype must be substitutable for its base without breaking callers (the classic Square/Rectangle violation).
...
- **vtables**: most OO languages put virtual methods in a per-class table; a call indirects through it (one extra pointer hop). Final/non-virtual methods can be statically bound or inlined.
...
- **Recursion** over mutation-driven loops; **tail-call optimization** where the runtime supports it (Scheme yes, Python/Java no — beware stack overflow).
...
- **Algebraic data types (ADTs)** — sum types (tagged unions: `Result = Ok | Err`) and product types (tuples/records). Pattern matching destructures them exhaustively. Rust enums, Scala/Kotlin sealed classes, Haskell `data`, TS discriminated unions.
...
- Deep recursion without TCO → stack overflow (Python, JVM). Rewrite as iteration or use trampolining.
...
- "`final` makes it immutable." Only the reference.
