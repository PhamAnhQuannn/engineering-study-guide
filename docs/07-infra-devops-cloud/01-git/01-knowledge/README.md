...
Lets you `pick / reword / edit / squash / fixup / drop / reorder` commits. Used to curate a messy local branch into a clean, reviewable series before opening/merging a PR. `git commit --fixup=<sha>` + `git rebase -i --autosquash` automates squashing fixups into their targets.
...
- **GitHub/GitLab flow** — feature branch → PR/MR → review → merge to `main`; `main` is always deployable.
- **Git flow** — `develop` + `main` + `release/*` + `hotfix/*` + `feature/*`. Heavy; suited to scheduled, versioned releases (e.g., installed software), overkill for web services with CD.
