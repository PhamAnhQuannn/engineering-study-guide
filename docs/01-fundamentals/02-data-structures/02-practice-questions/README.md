...
**Checklist**
- [ ] Distinguishes per-op worst case (O(n) on resize) from amortized O(1)
- [ ] Explains geometric growth → total copies < 2n
- [ ] Notes the tail-latency implication of the occasional O(n) spike
...
**Checklist**
- [ ] Hashmap: average O(1), no ordering
- [ ] Tree: O(log n) but ordered (range/floor/ceiling/iteration)
- [ ] Mentions the tradeoff (worst-case predictability vs speed)
...
**Checklist**
- [ ] O(1) splice only applies when you already hold the node
- [ ] Finding the position is still O(n); pointer-chasing hurts cache
- [ ] Gives a real win case (e.g. LRU) vs array cache locality
...
**Checklist**
- [ ] Strengths: O(1) peek, O(log n) push/pop, array layout
- [ ] Weaknesses: arbitrary search O(n), no ordered iteration/range
- [ ] Notes decrease-key needs an index map
...
**Checklist**
- [ ] High fan-out → shallow tree → fewer disk I/Os (node = page)
- [ ] Frames it as disk-seek cost, not just comparisons
...
**Checklist**
- [ ] States the guarantee: no false negatives, possible false positives
- [ ] Gives a use case (skip expensive lookup / SSTable check)
- [ ] Notes tuning (k hashes, bits) and no-delete limitation
...
**Checklist**
- [ ] Collisions → one bucket becomes a linear scan
- [ ] Names a cause (weak hash / hash-flooding attack)
- [ ] Names a defense (randomized hash, load factor, treeify)
...
**Checklist**
- [ ] Picks C
- [ ] Explains front-insert shifts n elements → O(n)
...
**Checklist**
- [ ] Picks B (min-heap of size k)
- [ ] States O(n log k) time, O(k) bounded memory
- [ ] Says why A/C/D fail the memory bound
...
**Checklist**
- [ ] Picks B (trie)
- [ ] Explains prefix walk O(L) + enumerate completions
- [ ] Notes sorted-array binary search as an alternative
...
**Checklist**
- [ ] Picks B
- [ ] Matrix is O(V²) memory → only worth it when dense / O(1) edge checks
...
**Checklist**
- [ ] Hashmap for O(1) lookup + doubly linked list for recency order
- [ ] Explains O(1) splice-to-head using the node ref from the map
- [ ] Says why neither structure alone suffices
...
**Checklist**
- [ ] Per-insert ×n = O(n log n); Floyd build-heap = O(n)
- [ ] Explains why bottom-up sift-down sums to linear
- [ ] States when each applies (all-up-front vs incremental)
